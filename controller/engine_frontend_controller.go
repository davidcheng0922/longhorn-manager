...
func (efc *EngineFrontendController) syncEngineFrontend(key string) (err error) {
	...
	// switchover while the instance is being stopped can leave I/O frozen
	// (phase Preparing suspends I/O but Promoting, which resumes it, never runs).
	...
	defer func() {
		if !reflect.DeepEqual(existingEF.Status, ef.Status) {
			if _, err := m.ds.UpdateEngineFrontendStatus(ef); err != nil {
				m.logger.WithError(err).Error("Failed to update engine frontend status")
			}
		}
	}()
