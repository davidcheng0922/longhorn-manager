...
func (ec *EngineController) syncEngine(key string) (err error) {
	...
	if engine.DeletionTimestamp != nil {
		...
		// For v2, wait for the SPDK RAID bdev to be fully torn down before
		// removing the finalizer. This prevents the volume controller from
		// proceeding to replica deletion while the RAID still references
		// the replica lvol bdevs.
		if types.IsDataEngineV2(engine.Spec.DataEngine) {
			...
			if _, err := ec.ds.UpdateEngineStatus(engine); err != nil {
				return err
			}
			if engine.Status.CurrentState != longhorn.InstanceStateStopped &&
				engine.Status.CurrentState != longhorn.InstanceStateError {
				log.Infof("Waiting for engine instance to stop before removing finalizer (current state: %v)", engine.Status.CurrentState)
				return nil
			}
		}
