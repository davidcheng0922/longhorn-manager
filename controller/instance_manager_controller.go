...
func (imc *InstanceManagerController) syncInstanceManagerUpgrade(im *longhorn.InstanceManager) error {
	...
	existing, existingErr := imc.ds.GetInstanceManagerUpgradeControl(types.InstanceManagerUpgradeControlName)
	...
	// Check if the upgrade has already started (any node not in pending state).
	upgradeStarted := false
	for _, info := range existing.Status.Nodes {
	...
	needsUpdate := false
	if existing.Spec.TargetImage != defaultIMImage {
		log.Infof("Updating InstanceManagerUpgradeControl target image from %v to %v",
			existing.Spec.TargetImage, defaultIMImage)
		existing.Spec.TargetImage = defaultIMImage
		needsUpdate = true
	}

	// Only update start time if the upgrade hasn't started yet.
	if !upgradeStarted && existing.Spec.StartAt != startTime {
		log.Infof("Updating InstanceManagerUpgradeControl start time from %v to %v",
			existing.Spec.StartAt, startTime)
		existing.Spec.StartAt = startTime
		needsUpdate = true
	}

	if needsUpdate {
		if _, err := imc.ds.UpdateInstanceManagerUpgradeControl(existing); err != nil {
			return err
		}
	}
	return nil
