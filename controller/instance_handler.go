...
func (h *InstanceHandler) syncStatusWithInstanceManager(log *logrus.Entry, obj runtime.Object, im *longhorn.InstanceManager, instanceName string, spec *longhorn.InstanceSpec, status *longhorn.InstanceStatus, instances map[string]longhorn.InstanceProcess) {
	...
	tolerateIMUnavailable := h.shouldTolerateEngineFrontendIMUnavailableInSplitTopology(log, obj, spec, status)
...
func (h *InstanceHandler) shouldTolerateEngineFrontendIMUnavailableInSplitTopology(log *logrus.Entry, obj runtime.Object, spec *longhorn.InstanceSpec, status *longhorn.InstanceStatus) bool {
	...
	if err != nil {
		if !datastore.ErrorIsNotFound(err) {
			log.WithError(err).Warnf("Failed to get volume %v while evaluating engine frontend IM tolerance", spec.VolumeName)
		}
		return false
	}
