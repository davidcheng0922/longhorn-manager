...
func (s *TestSuite) TestReconcileEngineFrontendToleratesSourceIMUnavailableAfterEngineSwitch(c *C) {
	testCases := map[string]struct {
		instanceManager *longhorn.InstanceManager
		volume          *longhorn.Volume
		expectedState   longhorn.InstanceState
	}{
		"source attachment node IM stopped after engine switched keeps unknown": {
		...
		"source attachment node IM is deleting after engine switched keeps unknown": {
			instanceManager: func() *longhorn.InstanceManager {
		...
		"source attachment node IM missing frontend process after engine switched keeps unknown": {
			instanceManager: newInstanceManager(
				TestInstanceManagerName, longhorn.InstanceManagerStateRunning,
				TestOwnerID1, TestNode1, TestIP1,
				map[string]longhorn.InstanceProcess{},
		...
		"split topology during switchover keeps unknown when source IM is unavailable": {
			instanceManager: newInstanceManager(
				TestInstanceManagerName, longhorn.InstanceManagerStateStopped,
				TestOwnerID1, TestNode1, TestIP1,
				map[string]longhorn.InstanceProcess{},
		...
		"without split topology source IM unavailable still becomes error": {
			instanceManager: newInstanceManager(
				TestInstanceManagerName, longhorn.InstanceManagerStateStopped,
				TestOwnerID1, TestNode1, TestIP1,
				map[string]longhorn.InstanceProcess{},
